...
def news_card(item: dict, highlight_fn=None) -> None:
    ...
    color = CATEGORY_COLORS.get(cat, "#374151")
    url   = item.get("url", "")
    headline = hl(item.get("headline", item.get("title", "")))
    summary  = hl(item.get("summary", ""))
    so_what  = hl(item.get("so_what", ""))
    ...
    with st.container():
        ...
        st.markdown(
            f'<a href="{url}" target="_blank" style="font-size:16.5px;font-weight:700;'
        ...
        if so_what:
            st.markdown(
                ...
                f'padding:10px 14px;border-radius:0 3px 3px 0;margin-bottom:4px">'
                ...
                f'<span style="font-size:13px;line-height:1.65;color:#1E3A5F">{so_what}</span>'
                f'</div>',
                unsafe_allow_html=True,
            )
        st.markdown(
